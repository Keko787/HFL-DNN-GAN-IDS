...
def test_write_figures_smoke(tmp_path: Path):
    ...
    # The paired-tests CSV + paper-ready LaTeX table sidecar + per-arm
    # summary table plus at least three of the six figures.
    assert "exp3_paired_tests.csv" in written_names
    assert "exp3_paired_tests.tex" in written_names
    assert "exp3_summary_table.tex" in written_names
    ...
    assert "exp3_fig0b_round_participation_rate.png" in written_names
